 ...
 * 
 * Includes all of the following: TweenLite, FWDRLTweenMax, TimelineLite, TimelineMax, EasePack, CSSPlugin, RoundPropsPlugin, BezierPlugin, AttrPlugin, DirectionalRotationPlugin
 *
...
(window._gsQueue || (window._gsQueue = [])).push( function() {
	...
	window._gsDefine("FWDRLTweenMax", ["core.Animation","core.SimpleTimeline","TweenLite"], function(Animation, SimpleTimeline, TweenLite) {
		
		var _slice = [].slice,
			FWDRLTweenMax = function(target, duration, vars) {
			...
			p = FWDRLTweenMax.prototype = TweenLite.to({}, 0.1, {}),
			_blankArray = [];

		FWDRLTweenMax.version = "1.9.7";
		p.constructor = FWDRLTweenMax;
		p.kill()._gc = false;
		FWDRLTweenMax.killTweensOf = FWDRLTweenMax.killDelayedCallsTo = TweenLite.killTweensOf;
		FWDRLTweenMax.getTweensOf = TweenLite.getTweensOf;
		FWDRLTweenMax.ticker = TweenLite.ticker;
...
//---- STATIC FUNCTIONS -----------------------------------------------------------------------------------------------------------
		...
		FWDRLTweenMax.to = function(target, duration, vars) {
			return new FWDRLTweenMax(target, duration, vars);
		};
		
		FWDRLTweenMax.from = function(target, duration, vars) {
			vars.runBackwards = true;
			vars.immediateRender = (vars.immediateRender != false);
			return new FWDRLTweenMax(target, duration, vars);
		};
		
		FWDRLTweenMax.fromTo = function(target, duration, fromVars, toVars) {
			toVars.startAt = fromVars;
			toVars.immediateRender = (toVars.immediateRender != false && fromVars.immediateRender != false);
			return new FWDRLTweenMax(target, duration, toVars);
		};
		
		FWDRLTweenMax.staggerTo = FWDRLTweenMax.allTo = function(targets, duration, vars, stagger, onCompleteAll, onCompleteAllParams, onCompleteAllScope) {
			stagger = stagger || 0;
			...
			for (i = 0; i < l; i++) {
				...
				a[i] = new FWDRLTweenMax(targets[i], duration, copy);
				delay += stagger;
			}
			return a;
		};
		
		FWDRLTweenMax.staggerFrom = FWDRLTweenMax.allFrom = function(targets, duration, vars, stagger, onCompleteAll, onCompleteAllParams, onCompleteAllScope) {
			vars.runBackwards = true;
			vars.immediateRender = (vars.immediateRender != false);
			return FWDRLTweenMax.staggerTo(targets, duration, vars, stagger, onCompleteAll, onCompleteAllParams, onCompleteAllScope);
		};
		
		FWDRLTweenMax.staggerFromTo = FWDRLTweenMax.allFromTo = function(targets, duration, fromVars, toVars, stagger, onCompleteAll, onCompleteAllParams, onCompleteAllScope) {
			toVars.startAt = fromVars;
			toVars.immediateRender = (toVars.immediateRender != false && fromVars.immediateRender != false);
			return FWDRLTweenMax.staggerTo(targets, duration, toVars, stagger, onCompleteAll, onCompleteAllParams, onCompleteAllScope);
		};
				
		FWDRLTweenMax.delayedCall = function(delay, callback, params, scope, useFrames) {
			return new FWDRLTweenMax(callback, 0, {delay:delay, onComplete:callback, onCompleteParams:params, onCompleteScope:scope, onReverseComplete:callback, onReverseCompleteParams:params, onReverseCompleteScope:scope, immediateRender:false, useFrames:useFrames, overwrite:0});
		};
		
		FWDRLTweenMax.set = function(target, vars) {
			return new FWDRLTweenMax(target, 0, vars);
		};
		
		FWDRLTweenMax.isTweening = function(target) {
		...
		var _getChildrenOf = function(timeline, includeTimelines) {
			...
			getAllTweens = FWDRLTweenMax.getAllTweens = function(includeTimelines) {
				return _getChildrenOf(Animation._rootTimeline, includeTimelines).concat( _getChildrenOf(Animation._rootFramesTimeline, includeTimelines) );
			};
		
		FWDRLTweenMax.killAll = function(complete, tweens, delayedCalls, timelines) {
			if (tweens == null) {
				tweens = true;
			}
		...
		FWDRLTweenMax.killChildTweensOf = function(parent, complete) {
			if (parent == null) {
				return;
			}
			...
			if (parent instanceof Array) {
				i = parent.length;
				while (--i > -1) {
					FWDRLTweenMax.killChildTweensOf(parent[i], complete);
				}
				return;
			}
			a = [];
		...
		FWDRLTweenMax.pauseAll = function(tweens, delayedCalls, timelines) {
			_changePause(true, tweens, delayedCalls, timelines);
		};
		
		FWDRLTweenMax.resumeAll = function(tweens, delayedCalls, timelines) {
...
//---- GETTERS / SETTERS ----------------------------------------------------------------------------------------------------------
		...
		p.duration = function(value) {
			if (!arguments.length) {
				return this._duration; //don't set _dirty = false because there could be repeats that haven't been factored into the _totalDuration yet. Otherwise, if you create a repeated FWDRLTweenMax and then immediately check its duration(), it would cache the value and the totalDuration would not be correct, thus repeats wouldn't take effect.
			}
...
(function(window) {
		...
		var _globals = window.GreenSockGlobals || window,
			 ...
			 * </script>
			 * <script src="js/greensock/v1.7/FWDRLTweenMax.js"></script>
			 * <script>
			 *     window.GreenSockGlobals = null; //reset it back to null so that the next load of FWDRLTweenMax affects the window and we can reference things directly like TweenLite.to(...)
			 * </script>
			 * <script src="js/greensock/v1.6/FWDRLTweenMax.js"></script>
			 * <script>
...
//----Animation getters/setters --------------------------------------------------------
		...
		p.duration = function(value) {
			...
			this._uncache(true); //true in case it's a FWDRLTweenMax or TimelineMax that has a repeat - we'll need to refresh the totalDuration.
